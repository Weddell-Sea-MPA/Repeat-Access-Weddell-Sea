################################################################################################
#
#                         SCRIPT 2: CALCULATE REPEATED ACCESSIBILITY (RA)
#
################################################################################################
#
#   (c) Flavia C B Trigo, Aug 2025; Adapted by Gemini for 2-stage workflow
#   Last updated/edited on 21/Aug/2026
#
#   PURPOSE:
#   This is the second and final stage of the analysis. It uses the simple
#   binary "accessibility maps" created by Script 08, step 1 to calculate the
#   final RA metrics (RA Quality and RA Existence) for RB4.
#
#   This script is very fast. If new years of data are added, you should:
#   1. Re-run Script 01 to process only the new data.
...
#     !!! These parameters MUST match in Script 08 (step 1) !!!
#
################################################################################################
message("\n--- Configuration ---")
...
# Dynamically routes results to the current year's folder for RB4.
storage_path = fs::path(project_root, "04_cleaned_data", current_year, "new_CRS_results_amsr2_48_6_4") |>
  fs::path_abs()
...
# - `seq()` generates a sequence of numbers from the start year to the end year.
years_to_analyze = seq(from=2012, to=as.numeric(current_year), by=1)

# 4. Define the ice concentration threshold (in percent) to determine accessibility. This must match Script 01.
threshold = 20 # requested by the Japanese
...
# Path to the folder containing the binary accessibility maps from Script 08.
intermediate_path = fs::path(storage_path, "01_intermediate_accessibility_maps") |>
  fs::path_abs()

# Path to the folder where final RA raster maps will be saved.
final_results_path = fs::path(storage_path, "02_final_RA_rasters") |>
  fs::path_abs()
fs::dir_create(final_results_path)

# Path to a folder for storing checkpoint files, which track completed tasks.
checkpoint_path = fs::path(storage_path, "03_checkpoints_final_RA") |>
  fs::path_abs()
fs::dir_create(checkpoint_path)

# Check if the input directory from Script 08 exists. If not, stop the script with an error message.
if (!fs::dir_exists(intermediate_path)) stop("Intermediate maps directory not found. Please run Script 08 first.")
  ...
  furrr::future_pwalk(
    ...
    .f = function(month_val, day_val, checkpoint_flag) {
      ...
      tryCatch({
        ...
        # Create a checkpoint file to mark this date as successfully processed.
...
message("\n#####################################################")
message("  Stage 2 Complete. All final RA maps created. ✅")
message("#####################################################")